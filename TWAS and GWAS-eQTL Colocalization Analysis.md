...
### TWAS Results Interpretation

#### Significance Thresholds

**Bonferroni Correction:**
```
Threshold = 0.05 / number_of_genes_tested
```

**Example:**
- 20,000 genes tested → threshold = 2.5 × 10⁻⁶
- Genes with p < 2.5 × 10⁻⁶ are genome-wide significant

#### Effect Size (Beta)

- **Positive beta**: Higher expression associated with higher trait values
- **Negative beta**: Higher expression associated with lower trait values
- **Magnitude**: Indicates strength of association
  - |beta| > 0.5: Strong effect
  - 0.2 < |beta| < 0.5: Moderate effect
...
**Interpretation:**
"CsaV4_3G003710 shows genome-wide significant association (p = 1.23 × 10⁻⁸) with fruit length. A one-unit increase in log₂(expression) is associated with a 0.234 cm decrease in fruit length."

---

### Colocalization Results Interpretation

#### Posterior Probability (PP) Thresholds

| PP.H4 Value | Interpretation | Confidence |
|-------------|----------------|------------|
| > 0.8 | Very strong evidence of colocalization | High |
| 0.5 - 0.8 | Strong evidence of colocalization | Moderate-High |
| 0.3 - 0.5 | Moderate evidence | Moderate |
| < 0.3 | Weak or no evidence | Low |

#### Five Hypotheses

**H0:** Neither trait has genetic association at this locus
- **PP.H0 > 0.5**: No genetic signal for either trait (likely false positive)

**H1:** Only GWAS trait has genetic association
- **PP.H1 > 0.5**: GWAS signal present, no eQTL (causal gene likely elsewhere)

**H2:** Only eQTL has genetic association
- **PP.H2 > 0.5**: eQTL present, no GWAS signal (gene regulated but not causal for trait)

**H3:** Both traits have associations, but different causal variants
- **PP.H3 > 0.5**: Two independent signals at same locus (linkage)

**H4:** Both traits share the same causal variant (colocalization)
- **PP.H4 > 0.5**: Strong evidence that gene expression mediates trait effect

#### Decision Tree

```
Is PP.H4 > 0.8?
├─ YES → Strong colocalization
│         Gene expression likely mediates trait effect
│         High-priority candidate gene
│
└─ NO → Check PP.H3
    ├─ PP.H3 > 0.5 → Two independent signals
    │                 Gene may not be causal
    │
    └─ PP.H1 or PP.H2 > 0.5 → Only one signal
                               Gene less likely causal
```

#### Example Interpretation

**Case 1: Strong Colocalization**
```
SNP: DEL28770
Gene: CsaV4_3G003710
PP.H4: 0.864 (86.4%)
...
**Interpretation:**
"There is strong evidence (PP.H4 = 86.4%) that the GWAS and eQTL signals at DEL28770 are driven by the same causal variant. CsaV4_3G003710 is a high-priority candidate gene, with its expression likely mediating the effect on fruit length."

**Case 2: Weak Colocalization**
```
SNP: INS22633
Gene: CsaV4_3G003710
PP.H4: 0.397 (39.7%)
...
**Interpretation:**
"Evidence for colocalization is moderate (PP.H4 = 39.7%), with substantial probability of independent signals (PP.H3 = 23.4%). Additional evidence (e.g., functional validation) needed to confirm CsaV4_3G003710 as causal gene."
...
1. **Expression Filtering**
   - Remove lowly expressed genes (TPM < 1 in < 5% samples)
   - Reduces multiple testing burden
   - Focuses on biologically relevant genes

2. **Normalization**
   - Use TPM normalization for RNA-seq
   - Apply quantile normalization (qqnorm)
   - Log-transform to stabilize variance

3. **Population Structure**
   - Always use kinship matrix
   - EMMAX effectively controls for structure
   - Check inflation factor (λ) should be ~1.0

4. **Sample Size**
   - Minimum 50 samples recommended
   - Extreme phenotype sampling can increase power
   - Balance between sample size and phenotypic variance
...
1. **Region Definition**
   - Use LD-based merging (typical: 1 Mb)
   - Include all variants in LD with lead SNP
   - Ensure sufficient overlap between GWAS and eQTL

2. **Sample Size**
   - COLOC requires accurate sample sizes
   - Use actual sample sizes (not effective sample size)
   - Same samples for GWAS and eQTL preferred

3. **Multiple Testing**
   - Test all genes in region
   - Apply FDR correction across genes
   - Report both gene-level and SNP-level PP.H4

4. **Validation**
   - PP.H4 > 0.8 is strong but not definitive
   - Consider:
...
#### TWAS: Kinship Matrix Error

**Problem:** EMMAX fails with singular matrix error

**Solutions:**
- Remove duplicated samples
- Increase MAF threshold (e.g., > 0.05)
- Check for excessive relatedness

#### TWAS: No Significant Results

**Problem:** No genes reach significance threshold

**Solutions:**
- Check expression normalization
- Verify phenotype distribution
- Increase sample size
- Consider gene-set analysis instead

#### Colocalization: Low PP Values

**Problem:** All PP values < 0.3

**Solutions:**
- Check sufficient overlap between datasets
- Verify MAF consistency
- Ensure proper p-value calculation
- Consider alternative window size

#### Colocalization: High PP.H3

**Problem:** PP.H3 > PP.H4 (independent signals)

**Interpretation:**
- Two causal variants in LD
- Gene may not be causal
- Consider conditional analysis
- Test alternative genes in region
...
| File | Description |
|------|-------------|
| `*.Trait.txt` | Extracted phenotype data |
| `*.Trait.pole.txt` | Filtered phenotype (top samples) |
| `*.plink.BN.kinf` | Balanced kinship matrix |
| `TPM.qqnorm.ordered.transposed.xls` | Ordered expression matrix |
| `*.TWAS.xls` | Final TWAS results |

### Colocalization Outputs

| File | Description |
|------|-------------|
| `*.LD.ld` | LD analysis results |
| `*.LD.ld.merge.bed` | Merged LD regions |
| `*.1MB.GWAS.txt` | GWAS summary in region |
| `*.1MB.gene.id` | Genes in region |
| `*.1MB.eQTL.txt` | eQTL results in region |
| `*.PP.H4.txt` | Colocalization results |
...
- Corresponding Author: [See publication]
